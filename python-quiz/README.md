...
1. Submit the prompt: `Create a pytest test suite for the code in the @python-quiz/ folder. Write tests for check_answer, calculate_score, format_result, and display_score. Cover normal cases and edge cases. Save the test file as python-quiz/tests.py.`

    > If you manually type `@` followed by the folder or file name, GitHub Copilot will give you a list to select from based on the files in the workspace.
...
1. Once you approve the plan, Copilot will create a `test_quiz.py` file. If Copilot doesn't request to run the test, you can do so manually:
    ...
    ```bash
    ! python -m pytest python-quiz/tests.py -v
    ```
...
The workflow you just used, `Document → Plan → Test → Fix`, is the same loop developers use. Copilot just helped you do it in roughly 10 minutes!